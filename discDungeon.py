...
def updateSaws():
    for saw in saws:
        ...
        if saw.active < 60: # if inactive then add to active timer
            saw.active += 1
        # player death
        elif saw.x-2 <= math.ceil(sprPlayer.x) and saw.x + 7 >= math.ceil(sprPlayer.x) and saw.y-2 <= math.ceil(sprPlayer.y) and saw.y + 7 >= math.ceil(sprPlayer.y):
            die()
...
def updateTargets():
    for tar in targets:
        ...
        if tar.lifespan <= 0:
            targets.remove(tar)
            continue
        if tar.x-2 <= math.ceil(sprPlayer.x) and tar.x + 7 >= math.ceil(sprPlayer.x) and tar.y-2 <= math.ceil(sprPlayer.y) and tar.y + 7 >= math.ceil(sprPlayer.y):
            global score
            score += 10
